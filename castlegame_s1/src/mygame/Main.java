...
public class Main extends SimpleApplication implements ActionListener {
    ...
    // SHOOTING STUFF:
    final int MAXBULLETS = 100;
    Sphere bulletsphere;
    Geometry geombullet[] = new Geometry[MAXBULLETS];
    BulletControl bulletcontrol[] = new BulletControl[MAXBULLETS];
    int bulletamt;
    boolean bulletfired;
    int bulletcounter[] = new int[MAXBULLETS];
    // ONSCREEN TEXT:
    BitmapText helpLine[];
    BitmapText hudLine[];
    // WEAPONS:
    Weapon weapons[] = {new Weapon(Weapon.weapon.sword, "sword", true, 1000, 50, 50),
                        new Weapon(Weapon.weapon.bow, "bow", true, 100, 150, 500)};
    int currentweapon = 0;
    ...
    public void simpleUpdate(float tpf) {
        ...
        for (int ix = 0; ix < MAXBULLETS; ix++)
        {
            if (bulletcontrol[ix] != null && geombullet[ix] != null)
            {
            if ((bulletcounter[ix]-- <= 0) || bulletcontrol[ix].collidedWithMap) {
                rootNode.detachChild(geombullet[ix]);
                getPhysicsSpace().remove(bulletcontrol[ix]);
                bulletfired = false;
            }
            }
        }
    ...
    public void onAction(String binding, boolean value, float tpf) {
        ...
        if (binding.equals("Fire")) {
            if (value) {
                if (!bulletfired && (weapons[currentweapon].ammo > 0)) {
                    ...
                    //bulletnode = new Node();
                    if (++bulletamt >= MAXBULLETS)
                    {
                        bulletamt = 0;
                    }
                    geombullet[bulletamt] = new Geometry("Bullet", bulletsphere);
                    Material mat = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
                    mat.setTexture("ColorMap", assetManager.loadTexture("Textures/circuit1.jpg"));
                    geombullet[bulletamt].setMaterial(mat);
                    //bulletnode.attachChild(geombullet);
                    bulletcontrol[bulletamt] = new BulletControl(sphereShape, 1f, bulletAppState);
                    getPhysicsSpace().add(bulletcontrol[bulletamt]);
                    geombullet[bulletamt].addControl(bulletcontrol[bulletamt]);
                    rootNode.attachChild(geombullet[bulletamt]);
                    //bulletcontrol.setKinematic(true);
                    bulletcontrol[bulletamt].setGravity(Vector3f.ZERO);
                    bulletcontrol[bulletamt].setPhysicsLocation(cam.getLocation().add(cam.getDirection().mult(2f)));
                    bulletcontrol[bulletamt].setLinearVelocity(cam.getDirection().mult(weapons[currentweapon].getLinearVelocity()));
                    weapons[currentweapon].ammo--;
                    bulletfired = true;
                    bulletcounter[bulletamt] = weapons[currentweapon].getCounter();
    ...
    public void showHelp() {
        String helpText[] = {
            ...
            "[ = rotate left",
            "] = rotate right",
            "B = fill rect: first press places anchor block, second press after moving cursor draws from the ",
            "anchor block to the current location, filling a rectangular prism of blocks",
            "ENTER = place/remove block",
            "PLUS KEY = increment current texture to place",
            "MINUS KEY = decrement current texture to place",
            "F1 = Toggle help",
            "F12 = Place player in the map roughly at the location of the cursor",
            "F5 = Save the map to \"themap.txt\"",
            "F7 = Load the map from \"themap.txt\"",
            "FPS CONTROLS:",
            "W = walk forward",
            "S = walk backward",
            "A = walk to left",
        ...
        helpLine = new BitmapText[helpText.length];

        for (int cx = 0; cx < helpText.length; cx++) {
            helpLine[cx] = new BitmapText(guiFont, false);
            helpLine[cx].setSize(guiFont.getCharSet().getRenderedSize());      // font size
            helpLine[cx].setColor(ColorRGBA.White);                             // font color
            helpLine[cx].setText(helpText[cx]);             // the text
            helpLine[cx].setLocalTranslation(0, -64 + settings.getHeight() - helpLine[cx].getLineHeight() * cx, 0); // position
            guiNode.attachChild(helpLine[cx]);
        }
    }
    
    public void hideHelp() {
        for (int cx = 0; cx < helpLine.length; cx++) {
            guiNode.detachChild(helpLine[cx]);
        }
    }
    ...
    public void showHud() {
        ...
        hudLine = new BitmapText[helpText.length];
